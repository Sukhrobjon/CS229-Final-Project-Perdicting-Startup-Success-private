...
class FoundersProcessor(BaseProcessor):
    ...
    def setup_logging(self):
        ...
        log_file = os.path.join(
            log_dir, 
            f'founders_processor_{datetime.now().strftime("%Y%m%d_%H%M")}.log'
        )
        ...
        error_log = os.path.join(
            log_dir, 
            f'founders_errors_{datetime.now().strftime("%Y%m%d_%H%M")}.log'
        )
    ...
    def get_founders(self, company_id: str) -> Optional[Dict]:
        """Get company founders with enhanced error handling and retry logic"""
        max_retries = 3
                    ...
                    if data:
                        if 'founders' in data:
                            ...
                            self.logger.info(
                                ...
                                f"Found {len(data['founders'])} founders. "
                                f"Time: {request_time:.2f}s"
                            )
                        return {company_id: data}  # Keep original structure with company_id as key
                    return None
    ...
    def emergency_merge_files(self):
        ...
        try:
            ...
            companies_with_founders = sum(
                1 for k, v in final_data.items() 
                if k != 'metadata' and v.get('founders') and len(v['founders']) > 0
            )
            total_companies = len(final_data) - 1  # Subtract 1 for metadata
            ...
            final_data['metadata'].update({
                ...
                "average_founders_per_company": self.total_founders / max(companies_with_founders, 1)
            })
            ...
            summary = {
                ...
                "Companies without founders": total_companies - companies_with_founders,
                "Total founders": self.total_founders,
                "Average founders per company": f"{self.total_founders / max(companies_with_founders, 1):.2f}"
            }
        ...
        try:
            ...
            if chunk_files:
                ...
                for i, chunk_file in enumerate(chunk_files, 1):
                    with open(chunk_file, 'r') as f:
                        chunk_data = json.load(f)
                        # Merge company data (excluding metadata if present)
                        for key, value in chunk_data.items():
                            if key != 'metadata':
                                final_data[key] = value
                    os.remove(chunk_file)
    ...
    def process_companies(self, company_ids: List[str]):
        ...
        try:
            # Process in batches of 100 companies
            batch_size = 100
            ...
            with ThreadPoolExecutor(max_workers=20) as executor:
                for i in range(start_index, total_companies, batch_size):
                    ...
                    futures = [
                        executor.submit(self.get_founders, company_id)
            ...
            try:
                if not os.path.exists(os.path.join(Config.OUTPUT_DIR, Config.OUTPUT_FILES['founders'])):
                    self.emergency_merge_files()
            except Exception as e:
