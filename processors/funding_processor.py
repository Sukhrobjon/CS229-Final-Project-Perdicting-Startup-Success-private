...
class FundingRoundsProcessor(BaseProcessor):
    ...
    def get_funding_rounds(self, company_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 429:  # Rate limit hit
                ...
                return self.get_funding_rounds(company_id)
            ...
            if response.status_code == 200:
                ...
                print(f"Error fetching funding rounds for {company_id}: {response.status_code}")
                return None
                
        except Exception as e:
            self.failed_calls += 1
            self.had_errors = True
            print(f"Exception fetching funding rounds for {company_id}: {str(e)}")
            return None
    ...
    def save_progress(self, current_index: int, company_id: str):
        ...
        progress_file = os.path.join(Config.OUTPUT_DIR, "funding_rounds_progress.json")
    ...
    def process_companies(self, company_ids: List[str]):
        """Process list of companies for funding rounds"""
        ...
        try:
            for i, company_id in enumerate(company_ids[start_index:], start_index + 1):
                try:
                    funding_data = self.get_funding_rounds(company_id)
                    if funding_data:
                        self.buffer[company_id] = funding_data
            ...
            # Final summary
            elapsed_time = time.time() - start_time
            print(f"\nFunding Rounds Processing Complete:")
            print("------------------------")
