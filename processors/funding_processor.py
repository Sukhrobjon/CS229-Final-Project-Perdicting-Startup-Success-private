...
class FundingRoundsProcessor(BaseProcessor):
    ...
    def get_funding_rounds(self, company_id: str, per_page: int = 10, page: int = 0) -> Optional[Dict]:
    ...
    def save_batch(self):
        """Save current batch of funding data"""
        filename = os.path.join(Config.OUTPUT_DIR, Config.OUTPUT_FILES['funding_rounds'])
    ...
    def process_companies(self, company_ids: List[str]):
        ...
        for i, company_id in enumerate(company_ids, 1):
            try:
                ...
                if funding_data:
                    self.data[company_id] = funding_data
        ...
        # Final summary
        elapsed_time = time.time() - start_time
        print(f"\nFunding Rounds processing completed:")
        print(f"Total companies processed: {total_companies}")
        print(f"Successful processes: {successful_processes}")
        print(f"Total time taken: {elapsed_time/60:.1f} minutes")
        print(f"Data saved to: {os.path.join(Config.OUTPUT_DIR, Config.OUTPUT_FILES['funding_rounds'])}")