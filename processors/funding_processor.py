# /processors/funding_processor.py
...
class FundingRoundsProcessor(BaseProcessor):
    def __init__(self, api_key: str, batch_size: int = 1000):
        ...
        # Progress tracking
        self.progress_file = os.path.join(Config.OUTPUT_DIR, "funding_rounds_progress.json")
    ...
    def setup_logging(self):
        ...
        self.logger = logging.getLogger('funding_rounds_processor')
        ...
        log_file = os.path.join(
            log_dir, 
            f'funding_rounds_{datetime.now().strftime("%Y%m%d_%H%M")}.log'
        )
        ...
        error_log = os.path.join(
            log_dir, 
            f'funding_rounds_errors_{datetime.now().strftime("%Y%m%d_%H%M")}.log'
        )
    ...
    def get_funding_rounds(self, company_id: str) -> Optional[Dict]:
        """Get funding rounds for a company with enhanced error handling and retry logic"""
        max_retries = 3
    ...
    def append_buffer_to_temp(self):
        ...
        temp_file = os.path.join(Config.OUTPUT_DIR, "funding_rounds_temp.json")
    ...
    def save_chunk_if_needed(self, companies_processed: int):
        ...
        if companies_processed % self.chunk_size == 0:
            ...
            temp_file = os.path.join(Config.OUTPUT_DIR, "funding_rounds_temp.json")
            chunk_file = os.path.join(Config.OUTPUT_DIR, f"funding_rounds_chunk_{chunk_number}.json")
    ...
    def merge_final_data(self):
        ...
        try:
            # Merge chunk files
            chunk_pattern = os.path.join(Config.OUTPUT_DIR, "funding_rounds_chunk_*.json")
            ...
            # Save final file
            final_file = os.path.join(Config.OUTPUT_DIR, Config.OUTPUT_FILES['funding_rounds'])
    ...
    def process_companies(self, company_ids: List[str]):
        ...
        self.logger.info(f"Starting Funding Rounds processing for {total_companies} companies...")
        print(f"\nStarting Funding Rounds processing for {total_companies} companies...")
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=20) as executor:
                for i in range(start_index, total_companies, batch_size):
                    ...
                    futures = [
                        executor.submit(self.get_funding_rounds, company_id)
            ...
            try:
                if not os.path.exists(os.path.join(Config.OUTPUT_DIR, Config.OUTPUT_FILES['funding_rounds'])):
                    self.emergency_merge_files()
            except Exception as e:
